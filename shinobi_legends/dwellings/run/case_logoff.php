...
//					ORDER BY $mu.userid DESC";
			...
			$hook=modulehook("dwellings-addsleepers",array("type"=>$row['type'],"dwid"=>$row['dwid'],"allowed"=>$allowed));
			...
			if($count>=$allowed){
			   output("There is no place for you to sleep here!  The following people are already taking up the available sleeping areas:`n`n");
				...
				for ($i = 0; $i < db_num_rows($result); $i++){
					...
					if($owner==1){
// This will break the translator!
//						rawoutput("<a href='runmodule.php?module=dwellings&op=kickout&who={$row['acctid']}&dwid=$dwid>");
//						output("`$ (Kick out)");
//						rawoutput("</a>");
				...
				if ($session['user']['loggedin']){
					...
					$sql = "UPDATE " . db_prefix("accounts") . " SET loggedin=0, location='Inside dwellings', restorepage='{$session['user']['restorepage']}' WHERE acctid = ".$session['user']['acctid'];
					db_query($sql);
