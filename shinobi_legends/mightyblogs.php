...
function mightyblogs_run(){
	...
	if ($id>"") {
		...
		if ($day > ""){
			...
			if ($time!='') $where .= " AND date>'".date("Y-m-d H",strtotime("-".$time))."'";
			$limit='';
		}
		...
		if ($day > ""){
			...
			if ($time!='') $where = "WHERE date>'".date("Y-m-d H",strtotime("-".$time))."'";
	...
	$sql = "SELECT " . db_prefix("accounts") . ".name, " . db_prefix("mod_mightyblogs") . ".* FROM " . db_prefix("mod_mightyblogs") . " INNER JOIN " . db_prefix("accounts") . " ON ". db_prefix("accounts") . ".acctid = ". db_prefix("mod_mightyblogs") . ".author $where ORDER BY date DESC $limit";
...
//	debug($sql); 
	...
	if ($op == "view") {
		...
		rawoutput("<td><a href='runmodule.php?module=mightyblogs&op=$op&author=$author&timeframe=1'>$sevendays</a><td>");
		addnav("","runmodule.php?module=mightyblogs&op=$op&author=$author&timeframe=1");
		$fourteendays=translate_inline("14 days");
		rawoutput("<td><a href='runmodule.php?module=mightyblogs&op=$op&author=$author&timeframe=2'>$fourteendays</a><td>");
		addnav("","runmodule.php?module=mightyblogs&op=$op&author=$author&timeframe=2");
		$onemonth=translate_inline("1 month");
		rawoutput("<td><a href='runmodule.php?module=mightyblogs&op=$op&author=$author&timeframe=3'>$onemonth</a><td>");
		addnav("","runmodule.php?module=mightyblogs&op=$op&author=$author&timeframe=3");
		$all=translate_inline("No Limit");
		rawoutput("<td><a href='runmodule.php?module=mightyblogs&op=$op&author=$author&timeframe=4'>$all</a><td>");
		addnav("","runmodule.php?module=mightyblogs&op=$op&author=$author&timeframe=4");
...
function mightyblogs_show($blog){
	...
	output_notl("`n");
	if ($session['user']['acctid']==$blog['author'] || ($session['user']['superuser']&SU_MEGAUSER) == SU_MEGAUSER){
