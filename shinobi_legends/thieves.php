...
function thieves_getmoduleinfo(){
	$info = array(
		...
		"version"=>"1.8",
		"author"=>"Shawn Strider, changes by `2Oliver Brendel",
		...
		"settings"=>array(
			...
			"elffriend"=>"Will Lonestrider help out the preferred race?,bool|0",
			"preferredrace"=>"Preferred Race he shows compassion with,text|Elf",
			...
			"lonestrider"=>"Name of Lonestrider,text|`\$Lonestrider",
			"lonestridersweapon"=>"Name of his weapon,text|Jewel-hilted Dagger",
			"thiefname"=>"Name of one of his thieves,text|`\$Lonestrider's Thief",
			"thiefweapon"=>"Name of the thief's weapon|Stabbing Knives",
...
function thieves_jewels(){
	...
	if ($selectjewel != "") output("`6While you were lying prone, %s`6 and his men took a fancy to your jeweled %s, and it is gone.`n`n",get_module_setting('lonestrider'),$selectjewel);
	debuglog("lost a $selectjewel to LoneStrider aka ".get_module_setting('lonestrider'));
	return true;
}
...
function thieves_elf_help($from) {
	...
	$gems = $session['user']['gems'];
	$op = httpget("op");
	$lonestrider=get_module_setting('lonestrider');
	...
	if ($gems == 0) {
		...
		output("Upon finding none, they bring you to their leader %s`6 to figure out what to do with you.`n`n",$lonestrider);
		output("%s `6bows slightly as soon as they push you before him.",$lonestrider);
		output("He glares at the bandits and shakes his head.`n");
		output("`\$\"I'm sorry about the mixup, these comrades of mine seem to have confused you for someone else. They tell me you're travelling without a gem to call your own. No %s should be without a single gem.  Please, consider this a gift.\"`n",get_module_setting('preferredrace'));
		...
		output("You curse yourself for not seeing them sooner, for this is none other than the infamous %s`3 and his pack of thieves.",$lonestrider);
		output("Your mind races through a handful of options before %s `3raises his hand in a show of good faith.`n`n",$lonestrider);
		output("`\$\"Greetings and salutations my fine %s. Wonderful day for a walk in the forest to solidify the foundations of our fortunes, isn't it?\"`3`n",get_module_setting('preferredrace'));
		output("You hear a dozen hidden thieves chime with laughter.`n");
		output("`\$\"Don't look so afraid my %s, I don't have any intention of robbing you. I try to protect other `6members of the %s`\$, when I can.  Well, I admit we run into an occasional dry spell where anyone will be asked to contribute to our wages, but I honour the elements and the fey blood in my heart.`nI couldn't help but to notice that you were looking a bit lost there a moment ago and I thought we might be able to help a fellow elvish adventurer.\"`n`n", $salut,get_module_setting('preferredrace'));
		output("`3Still a bit stunned at the speed of which these bandits overtook you, you consider your reply in silence.");
		output("You know that %s `3is normally worth quite a bit of money, dead or alive. . . if you can catch him.",$lonestrider);
		...
		output("\"`6Thank you %s`6, but I'm fine!\"",$lonestrider);
		rawoutput("</a><br/>");
		...
		/////////////////////////////////////////
		addnav(array("Answer %s`0?",sanitize($lonestrider)));
		...
		addnav(array("Kill %s!",sanitize($lonestrider)));
		...
		output("`n`n`6You anxiously tell %s`6 a short story of your adventures and mention that you're also a elvish thief, following in his footsteps.",$lonestrider);
		...
		output("Next thing you know, %s`6 is dangling your gem pouch in his hands with a mischievious smirk.",$lonestrider);
		...
		if ($session['user']['gems']>3*get_module_setting("mingems")){
			...
			debuglog("gained $money gold from $lonestrider for being an ".get_module_setting('preferredrace'));
		} else {
			output("`n`n`6Depressed, you tell %s`6 that you're having no luck finding `%gems`6.",$lonestrider);
			output("He seems to understand and nods his head with a small smile.`n`n");
...
function thieves_fight($costlose) {
	$op = httpget("op");
	global $session;
	rawoutput("<center><img src='modules/thieves/lonestrider.jpg' alt='".get_module_setting('lonestrider')."'></center>");
	if ($op=="stand"){
		...
		if ($usemultis == true) {
			...
			$lonestrider = array(
				"creaturename"=>get_module_setting('lonestrider'),
				"creaturelevel"=>$badguylevel,
				"creatureweapon"=>get_module_setting('lonestridersweapon'),
				...
				"type"=>"lonestrider");
			$thief = array(
				"creaturename"=>get_module_setting('thiefname'),
				"creaturelevel"=>$badguylevel,
				"creatureweapon"=>get_module_setting('thiefweapon'),
				"creatureattack"=>$badguyatt,
				"creaturedefense"=>$badguydef,
			...
			$badguy = array(
				"creaturename"=>get_module_setting('lonestrider').translate_inline("'s Thieves`0"),
				"creaturelevel"=>$badguylevel,
			...
			apply_buff('thieves', array(
				...
				"minioncount"=>min(10, $session['user']['level']),
				"mingoodguydamage"=>0,
	...
	if ($battle){
		...
		if ($victory){
			if (e_rand(1, 100) < get_module_setting("chancebackstab")) {
				...
				output("`n%s`6 thieves lay slain at your feet but the elfin leader has vanished.",get_module_setting('lonestrider'));
				output("Eagerly you glance about the area, looking for %s`6 himself.",get_module_setting('lonestrider'));
				...
				output("You wake up with a splitting headache and find `6that %s`6 has taken `%%s gems`6 from your unconcious body.",get_module_setting('lonestrider'),$costlose2);
				debuglog("lost $costlose2 gems to Lonestrider aka ".get_module_setting('lonestrider')."backstab");
				if (get_module_setting("losegold")) {
					$goldloss = $session['user']['gold'];
					$session['user']['gold'] = 0;
					output("Additionally, you find `^%s gold`6 gone.", $goldloss);
					debuglog("lost $goldloss gold to Lonestrider aka ".get_module_setting('lonestrider')."backstab");
				}
				require_once("lib/taunt.php");
				$taunt = select_taunt_array();
				addnews("`%%s`2 challenged %s `2and his band of thieves and put up a fierce fight! Unfortunately, %s `2got the last blow in.`n%s",$session['user']['name'],get_module_setting('lonestrider'),get_module_setting('lonestrider'), $taunt);
				$session['user']['specialmisc']="";
				$session['user']['specialinc']="";
			} else {
				output("`n`6Many of %s`6's thieves lay slain at your feet.",get_module_setting('lonestrider'));
				output("%s`6 himself has disappeared at some point in the battle, when things were looking sour for his men.`n",get_module_setting('lonestrider'));
				if (is_module_active("dag")) {
					...
					debuglog("gained $bounty gold for the head of some of Lonestrider's aka ".get_module_setting('lonestrider')."'s thieves");
				}
				...
				if ($session['user']['specialmisc']=="triedtorun") {
					...
					output("%s`6 must have taken them when he ran.",get_module_setting('lonestrider'));
				}
			...
			addnews("`%%s`6 challenged %s `6and his band of thieves, but was no match for the rogues!`n%s",$session['user']['name'],get_module_setting('lonestrider'),$taunt);
			debuglog("lost $costlose gems when Lonestrider's aka ".get_module_setting('lonestrider')."'s thieves knocked him unconscious.");
			output("`n%s's`6 thieves have laid you unconscious, and help themselves to the `%%s`6 gems that they find in one of your purses.",get_module_setting('lonestrider'), $costlose);
...
function thieves_runevent($type)
{
	...
	if ($session['user']['race'] == get_module_setting('preferredrace') &&
	...
	if ($gems<=get_module_setting("mingems")){
		...
		addnews("`3%s`6 got roughed up by %s `6 and his cronies for having little of value.",$session['user']['name'],$lonestrider);
		return;
	}
	...
	if ($op=="" || $op=="search"){
		...
		output("Their elven leader %s `6shouts above their jeers that you stand or deliver!",$lonestrider);
		...
		output("`n`n`6You realize that %s's`6 forces are more than a match for you, and fearing for your life, you elect to give them the `%%s`6 gems they have demanded.",$lonestrider, $costwin);
		$session['user']['gems']-=$costwin;
		debuglog("relinquished $costwin gems to pay off Lonestrider's aka ".$lonestrider."'s thieves");
		$session['user']['specialinc']="";
	}elseif ($op=="runawaylikealittlesissybaby"){
		output("`n`n`6You realize that %s's`6 forces are a greedy and easily distracted bunch, so you throw `%%s`6 %s at them and begin to run.",$lonestrider, $costrun, translate_inline(($costrun > 1)?"gems":"gem"));
		$session['user']['gems']-=$costrun;
		debuglog("tossed $costrun gems trying to distract Lonestrider's aka ".$lonestrider."'s thieves");
		if (e_rand(1,100) > get_module_setting("chancedistract")) {
			...
			output("%s`6 stops to gather up the %s before joining his jeering companions, prepared to fight.",$lonestrider, translate_inline(($costrun > 1)? "gems":"gem"));
