...
function titlechange_dohook($hookname,$args){
	...
	case "pointsdesc":
		$args['count']++;
		$format = $args['format'];
		$str = translate("The ability to choose a custom in-game title upon reaching %s and every %s points thereafter. (this doesn't use up those points) [NOTE: A title is the prefix on your name like Farmboy or Page]. You have done so %s times and now need at least %s points to do so again!");
	...
	case "lodge":
		...
		if ($need>=0) {
			...
			addnav(array("Set Custom Title (you need %s more points!)",$need),"");
		}	
		break;
	}
	return $args;
...
function titlechange_run(){
	...
	switch ($op) {
		...
		case "titlepreview":
			...
			$ntitle=titlechange_remove_emoji($ntitle);
			if ($ntitle=="") {
			...
			if (strlen($ntitle)>get_module_setting('length')) {
				output("`7Your new title is too long. Please shorten it.`n ");
				output("`7Your title looked like this: %s`0`n", $ntitle);
				rawoutput("<form action='runmodule.php?module=titlechange&op=titlepreview' method='POST'>");
				rawoutput("<input id='input' name='newname' width='25' maxlength='25' value=\"$ntitle\">");
				rawoutput("<input type='submit' class='button' value='Preview'>");
				rawoutput("</form>");
				break;
			}
...
function titlechange_remove_emoji($text){
