...
function stafflist_getmoduleinfo(){
	$info = array(
		"name"=>"Staff List",
		"version"=>"1.2",
		"author"=>"`\$Red Yates`0, modified by `2Oliver Brendel",
		"allowanonymous"=>true,
...
function stafflist_run(){
	...
	$sql="SELECT userid FROM ".db_prefix('module_userprefs')." WHERE modulename='stafflist' AND setting='inactive' AND value+0=1";
	$result=db_query($sql);
	$inactive=array();
	...
	if ($inactive!==array()) {
		$il=implode(",",$inactive);
		$inact="AND u.acctid NOT IN (".$il.")";
		$act="AND u.acctid IN (".$il.")";
	} else $act=" AND 0 ";

	$biolink=get_module_setting("biolink");
	$sql = "SELECT p1.userid, (p1.value+0) AS rank1, p2.value AS descr, u.name, u.login, u.sex, u.laston, u.loggedin FROM ".db_prefix("accounts")." as u, ".db_prefix("module_userprefs")." as p1, ".db_prefix("module_userprefs")." as p2 WHERE (p1.value+0) > 0 AND p1.modulename='stafflist' AND p1.setting='rank' AND p1.userid=u.acctid AND p2.modulename='stafflist' AND p2.setting='desc' AND p2.userid=u.acctid AND (u.superuser&".SU_IS_TRANSLATOR."!=u.superuser) $inact  ORDER BY rank1 DESC,descr DESC, u.acctid ASC";
	...
	if ($count>0){
		$notrans=translate_inline("Staff (Gameplay/Administrative)");
		rawoutput("<center>");
		...
		$sql = "SELECT p1.userid, (p1.value+0) AS rank1, p2.value AS descr, u.name, u.login, u.sex, u.laston, u.loggedin FROM ".db_prefix("accounts")." as u, ".db_prefix("module_userprefs")." as p1, ".db_prefix("module_userprefs")." as p2 WHERE (p1.value+0) > 0 AND p1.modulename='stafflist' AND p1.setting='rank' AND p1.userid=u.acctid AND p2.modulename='stafflist' AND p2.setting='desc' AND p2.userid=u.acctid AND (u.superuser&".SU_IS_TRANSLATOR."=u.superuser) $inact ORDER BY rank1 DESC,descr DESC, u.acctid ASC";
		$result = db_query_cached($sql, "stafflist-translators", 600);
		$count = db_num_rows($result);
		rawoutput("<center>");
		$trans=translate_inline("Translators");
		rawoutput("<table border='0' cellpadding='2' cellspacing='1' bgcolor='#999999'>");
		rawoutput("<tr bgcolor='#434325'><td colspan=4 align='center'>$trans</td>");
		rawoutput("</tr>");
		rawoutput("<tr class='trhead'><td>$hname</td><td>$hsex</td>");
		if ($showdesc) rawoutput("<td>$hdesc</td>");
		if ($showon) rawoutput("<td>$hon</td>");
		...
		//Inactives
		$sql = "SELECT p1.userid, (p1.value+0) AS rank1, p2.value AS descr, u.name, u.login, u.sex, u.laston, u.loggedin FROM ".db_prefix("accounts")." as u, ".db_prefix("module_userprefs")." as p1, ".db_prefix("module_userprefs")." as p2 WHERE (p1.value+0) > 0 AND p1.modulename='stafflist' AND p1.setting='rank' AND p1.userid=u.acctid AND p2.modulename='stafflist' AND p2.setting='desc' AND p2.userid=u.acctid $act  ORDER BY rank1 DESC,descr DESC, u.acctid ASC";
	$result = db_query_cached($sql, "stafflist-inactives", 600);
		$count = db_num_rows($result);
		rawoutput("<center>");
		$trans=translate_inline("Inactives");
		rawoutput("<table border='0' cellpadding='2' cellspacing='1' bgcolor='#999999'>");
		rawoutput("<tr bgcolor='#434325'><td colspan=4 align='center'>$trans</td>");
		rawoutput("</tr>");
		rawoutput("<tr class='trhead'><td>$hname</td><td>$hsex</td>");
