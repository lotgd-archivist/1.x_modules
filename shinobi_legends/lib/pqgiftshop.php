...
case "send2":
	$sql = "SELECT login,name,level,acctid FROM ".db_prefix("accounts")." WHERE name LIKE '".$whom."' and acctid <> '".$session['user']['acctid']."' ORDER BY level,login ";
	$result = db_query($sql);
	...
	while ($row = db_fetch_assoc($result)) {
		...
		rawoutput("</td></tr>");
		addnav("","runmodule.php?module=".$shope."&op=send3&op2=".rawurlencode($gift)."&price=$price&name=".HTMLEntities($row['acctid']));
	}
	rawoutput("</table>");
	output_notl("`n");
	addnav("Go Back","runmodule.php?module=".$shope);
