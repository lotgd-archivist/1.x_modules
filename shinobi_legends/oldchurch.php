...
function oldchurch_getmoduleinfo() {
    $info = array(
        ...
        "settings"=>array(
			...
			"name"=>"Name of the male priest,text|Vicar Karasu",
        ),
        "prefs"=>array(
...
function oldchurch_run() {
	...
	if ($bloodgift==1) {
		...
		output("`5%s `^grins as you walk over to the donation plate, waiting to see how much you will put in before making any preparations.",$name);
		...
		if ($donation>$session['user']['gold'] || $donation < 0) {
			output("`5%s `^looks somewhat grim as you try to persuade him that it is the spirit of giving which counts.`n`n",$name);
			output("You are hastily ejected from the church.");
		} elseif ($donation>=($session['user']['level']*103)) {
			output("`^As you put a hefty amount of gold in the bowl, `5%s`^'s grin widens and he beckons you over to the altar for a blessing.",$name);
			...
			apply_buff('capelthwaite_blessing',
				array("name"=>array("`5%s's Blessing",$name),
					"rounds"=>15,
			...
			output("`^As the few coins you decided to spare rattle into the bowl, `5%s`^'s grin turns sickening as he beckons you over to the altar.`n`n",$name);
		...
		output("`3You nervously inquire about another way that you could earn a blessing as `5%s`3 looks on you magnanimously.`n`n",$name);
		...
		output("`^Trusting in `5%s`^'s friendly smile, you drink down the potion. You feel funny for a moment, then the world begins to swim before your eyes, and you black out.`n`n",$name);
		$ritual=e_rand(1,100);
		if ($ritual>=100) {
			...
			output("Eventually you are dragged back up through some tunnels like a sack of grain before being dumped in front of the altar, where `5%s`7 feeds you another potion.`n`n",$name);
		} else {
			output("You dream of smoke and a glowing light.`n`n");
		}
		output("`^You regain your faculties with `5%s`^ leaning over you, looking red-faced.",$name);
