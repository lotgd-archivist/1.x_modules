...
$ranks = translate_inline($args['ranks']);
	...
	if ($op==""||$op=="list"){
		...
		if ($op=="mview"){
			...
			$count=false;
			$offset=0;
			while (false!==strpos($claninfo['clanmotd'],"`c",$offset)) {
		
				$offset=strpos($claninfo['clanmotd'],"`c",$offset)+1;
				$count=!$count;
			}
			if ($count) output_notl("`c");
			$count=false;
			$offset=0;
			while (false!==strpos($claninfo['clanmotd'],"`b",$offset)) {
		
				$offset=strpos($claninfo['clanmotd'],"`b",$offset)+1;
				$count=!$count;
			}
			if ($count) output_notl("`b");	
			...
			$count=false;
			$offset=0;
			while (false!==strpos($claninfo['clandesc'],"`c",$offset)) {
		
				$offset=strpos($claninfo['clandesc'],"`c",$offset)+1;
				$count=!$count;
			}
			if ($count) output_notl("`c");
			$count=false;
			$offset=0;
			while (false!==strpos($claninfo['clandesc'],"`b",$offset)) {
		
				$offset=strpos($claninfo['clandesc'],"`b",$offset)+1;
				$count=!$count;
			}
			if ($count) output_notl("`b");			
			modulehook("}collapse");
			...
			if (httpget("sop")=="yes") {
				...
				$sql = "UPDATE ".db_prefix("accounts")." SET clanid=0,clanrank=".CLAN_APPLICANT.",clanjoindate='".DATETIME_DATEMIN."' WHERE clanid=$dt";
				db_query($sql);
			...
			if ($remove>""){
				$sql = "UPDATE " . db_prefix("accounts") . " SET clanrank=".CLAN_APPLICANT.",clanid=0,clanjoindate='".DATETIME_DATEMIN."' WHERE login='$remove' AND clanrank<={$session['user']['clanrank']}";
				db_query($sql);
			...
			while ($row=db_fetch_assoc($result)){
				...
				rawoutput("<td>");
				output_notl($ranks[$row['clanrank']]."(".$row['clanrank'].")");
				rawoutput("</td><td>");
