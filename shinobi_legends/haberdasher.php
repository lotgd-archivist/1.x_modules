...
function haberdasher_dohook($hookname, $args){
	...
	case "dragonkilltext":
		...
		if (($losepercent > 0) && ($hatgold > 0)){
			...
			$hatgold-=$loseamount;
			output("`n`\$Sadly, this horrible fight did do bad things to your gear - hence your hat suffered damage and is now worth `^%s`\$ gold pieces less than before.`n",$loseamount);
			...
			$oldhat=get_module_pref("hatsize");
			$hatsize=haberdasher_sizecalc($session['user']['acctid']);
			if ($oldhat>$hatsize) {
				output("`\$Your hat size has been decreased to `y%s`\$!`n",$hatsize);
			}
...
function haberdasher_run(){
	...
	if ($op=="buy") {
		...
		if ($session['user']['gold']==0){
			...
			increment_module_pref("hatgold", $amount);
			$hatsize=haberdasher_sizecalc($uid);
		...
		$sql= "SELECT COUNT(userid) AS c FROM " . db_prefix("module_userprefs") . " WHERE modulename='haberdasher' and value>0 and setting='hatsize'";
		$result = db_query($sql);
