...
function xmasshoppe_getmoduleinfo(){
	$info = array(
		"name"=>"Holiday Christmas Gift Shop (timelocked)",
		"author"=>"JT Traub",
...
function xmasshoppe_install(){
	require_once("lib/tabledescriptor.php");

	// See if we need to insert the basic gifts
	$populate = false;
	if (!db_table_exists(db_prefix("xmas_giftlist"))) {
		$populate = true;
	}
	...
	$giftlistdesc = array(
		...
		'key-PRIMARY' => array('name'=>'PRIMARY', 'type'=>'primary key',
			'unique'=>'1', 'columns'=>'giftid'));
	synctable(db_prefix("xmas_gifts"), $giftsdesc, true);
	synctable(db_prefix("xmas_giftlist"), $giftlistdesc, true);

	if ($populate) {
		// Create some basic gifts
		$basegifts = array(
			"INSERT INTO " . db_prefix("xmas_giftlist") . " VALUES (0, '`QHoliday Fruitcake`0', 50, 0)",
			"INSERT INTO " . db_prefix("xmas_giftlist") . " VALUES (0, '`@Plush Dragon`0', 100, 0)",
			"INSERT INTO " . db_prefix("xmas_giftlist") . " VALUES (0, '`&Comfy Pillow`0', 150, 0)",
			"INSERT INTO " . db_prefix("xmas_giftlist") . " VALUES (0, '`2Subscription to \"`@Dragon Killers Weekly`2\"`0', 200, 0)",
			"INSERT INTO " . db_prefix("xmas_giftlist") . " VALUES (0, '`!Fancy Quill Pen`0', 0, 1)"
		);
...
function xmasshoppe_uninstall() {
	debug("Dropping gifts table");
	$sql = "DROP TABLE IF EXISTS " . db_prefix("xmas_gifts");
//	db_query($sql);
	debug("Dropping giftlist table");
	$sql = "DROP TABLE IF EXISTS " . db_prefix("xmas_giftlist");
//	db_query($sql);

	return true;
}

function xmasshoppe_dohook($hookname,$args) {
	global $session;
	...
	case "fireplace":
		...
		$sql = "SELECT count(*) AS count FROM " . db_prefix("xmas_gifts") . " WHERE giftto='$id'";
		...
		if ($days > -7 && $days < 1) {
			if ($count) {
				...
				addnav("Open A Present",
						"runmodule.php?module=xmasshoppe&op=open");
			}
		} elseif ($days == 1) {
			if ($count > 1) {
	...
	case "newday":
		if ($days > -3 && $days < 1) {
				output("`c`\$Merry Christmas!!!`%`c`n`nThere might be presents for you at the fireplace! You might want to open them quickly!`n");
		}
		break;
	case "village":
		if ($days > -3 && $days < 1) {
		...
		if ($days <= get_module_setting("daysprior") && $days >= 0) {
			if ($session['user']['location']!='Sunagakure') break;
			tlschema($args['schemas']['marketnav']);
			addnav($args['marketnav']);
			tlschema();
			addnav("Ye Olde Gifte Shoppe", "runmodule.php?module=xmasshoppe");
		}
		break;
	case "superuser":
		if (get_module_pref("canedit")) {
			...
			addnav("Gifts Editor",
					"runmodule.php?module=xmasshoppe&op=editor&admin=true");
		}
		break;
	case "changesetting":
		if ($args['setting'] == "date" && $args['module']=="xmasshoppe") {
			// Someone changed the holiday date, remove the setting that
			// people already picked up their 'extra' gift for this holiday.
			$sql = "DELETE FROM " . db_prefix("module_userprefs") . " WHERE modulename='xmasshoppe' AND setting='gottengift'";
			db_query($sql);
		}
		break;
	}
...
function xmasshoppe_run(){
	...
	if ($op =="editor"){
		...
		if ($count >= 1) {
			$sql = "SELECT * FROM " . db_prefix("xmas_gifts") . " WHERE giftto='$id' ORDER BY RAND(".e_rand().") LIMIT 1";
			$row = db_fetch_assoc(db_query($sql));
			$gid = $row['giftid'];
			...
			$sql = "DELETE FROM " . db_prefix("xmas_gifts") . " WHERE giftid='$gid'";
			db_query($sql);
			$count--;
		} elseif (get_module_setting("extragift") && !get_module_pref("gottengift")) {
			set_module_pref("gottengift", 1);
			// Select a random gift
			$sql1 = "SELECT name FROM " . db_prefix("xmas_giftlist") . " ORDER BY RAND(".e_rand().") LIMIT 1";
		...
		villagenav();
		addnav("Return to Gifte Shoppe", "runmodule.php?module=xmasshoppe");
		$id = httpget("item");
		$sql = "SELECT * FROM " . db_prefix("xmas_giftlist") . " WHERE giftid='$id'";
		$row = db_fetch_assoc(db_query($sql));
		modulehook("xmasshoppe");
		if (!$row['giftid']) {
			...
			rawoutput("<form action='runmodule.php?module=xmasshoppe&op=checkname' method='POST'>");
			addnav("", "runmodule.php?module=xmasshoppe&op=checkname");
			output("`^Recipient: ");
		...
		if ($count == 0) {
			...
			addnav("Re-enter name",
					"runmodule.php?module=xmasshoppe&op=buy&item=$item");
		} elseif ($count > 100) {
			...
			rawoutput("<form action='runmodule.php?module=xmasshoppe&op=checkname&subfinal=1' method='POST'>");
			addnav("", "runmodule.php?module=xmasshoppe&op=checkname&subfinal=1");
			output("`^Recipient: ");
			...
			rawoutput("<form action='runmodule.php?module=xmasshoppe&op=send' method='POST'>");
			addnav("", "runmodule.php?module=xmasshoppe&op=send");
			rawoutput("<input type='hidden' value='".httppost('gift')."' name='gift'>");
			rawoutput("<input type='hidden' value='".httppost('gemcost')."' name='gemcost'>");
			rawoutput("<input type='hidden' value='".httppost('goldcost')."' name='goldcost'>");
			$recip = $row['acctid'];
		...
		$sql = "INSERT INTO " . db_prefix("xmas_gifts") . " (giftid,giftfrom,giftto,name,note,anonymous) VALUES(0,$from,$recip,'$gname','$note',$anon)";
		db_query($sql);
		...
		addnav("Gifts");
		$sql = "SELECT * FROM " . db_prefix("xmas_giftlist");
		$res = db_query($sql);
...
function xmasshoppe_editor(){
	...
	superusernav();
	addnav("Gift Editor");
	addnav("Add a gift","runmodule.php?module=xmasshoppe&op=editor&subop=add&admin=true");
	$subop = httpget('subop');
	$giftid = httpget('giftid');
	$header = "";
	if ($subop != "") {
		addnav("Gift Editor Main","runmodule.php?module=xmasshoppe&op=editor&admin=true");
		if ($subop == 'add') {
	...
	if($subop=="del"){
		$sql = "DELETE FROM " . db_prefix("xmas_giftlist") . " WHERE giftid='$giftid'";
		db_query($sql);
		$subop = "";
		httpset('subop', "");
	}
	if($subop=="save"){
		...
		if ($giftid > 0) {
			$sql = "UPDATE " . db_prefix("xmas_giftlist") . " SET $sql WHERE giftid='$giftid'";
		} else {
			$sql = "INSERT INTO " . db_prefix("xmas_giftlist") . " ($keys) VALUES ($vals)";
		}
		db_query($sql);
	...
	if ($subop==""){
		...
		rawoutput("</tr>");
		$sql = "SELECT * FROM " . db_prefix("xmas_giftlist") . " ORDER BY giftid";
		$result= db_query($sql);
		for ($i=0;$i<db_num_rows($result);$i++){
			$row = db_fetch_assoc($result);
			$id = $row['giftid'];
			rawoutput("<tr class='".($i%2?"trlight":"trdark")."'>");
			rawoutput("<td nowrap>[ <a href='runmodule.php?module=xmasshoppe&op=editor&subop=edit&giftid=$id&admin=true'>$edit</a>");
			addnav("","runmodule.php?module=xmasshoppe&op=editor&subop=edit&giftid=$id&admin=true");
			rawoutput(" | <a href='runmodule.php?module=xmasshoppe&op=editor&subop=del&giftid=$id&admin=true' onClick='return confirm(\"$conf\");'>$del</a> ]</td>");
			addnav("","runmodule.php?module=xmasshoppe&op=editor&subop=del&giftid=$id&admin=true");
	...
	if($subop=="edit"){
		$sql="SELECT * FROM " . db_prefix("xmas_giftlist") . " WHERE giftid='$giftid'";
		$result = db_query($sql);
		$row = db_fetch_assoc($result);
	}elseif ($subop=="add"){
	...
	if ($subop == "edit" || $subop == "add") {
		require_once("lib/showform.php");
		rawoutput("<form action='runmodule.php?module=xmasshoppe&op=editor&subop=save&admin=true' method='POST'>");
		addnav("","runmodule.php?module=xmasshoppe&op=editor&subop=save&admin=true");
