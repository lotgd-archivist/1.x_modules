...
function specialtyNinjutsu_getmoduleinfo(){
	$info = array(
		"name" => "Specialty - Ninjutsu",
		"author" => "Ann, based on Dark Arts by Eric Stevens",
		"version" => "1.0",
		"download" => "",
...
function specialtyNinjutsu_install(){
	$sql = "DESCRIBE " . db_prefix("accounts");
	$result = db_query($sql);
	$specialty="DA";
	while($row = db_fetch_assoc($result)) {
		// Convert the user over
		if ($row['Field'] == "Ninjutsu") {
			debug("Migrating Ninjutsu field");
			$sql = "INSERT INTO " . db_prefix("module_userprefs") . " (modulename,setting,userid,value) SELECT 'specialtyNinjutsu', 'skill', acctid, Ninjutsu FROM " . db_prefix("accounts");
			db_query($sql);
			debug("Dropping Ninjutsu field from accounts table");
			$sql = "ALTER TABLE " . db_prefix("accounts") . " DROP darkarts";
			db_query($sql);
		} elseif ($row['Field']=="Ninjutsuuses") {
			debug("Migrating darkarts uses field");
			$sql = "INSERT INTO " . db_prefix("module_userprefs") . " (modulename,setting,userid,value) SELECT 'specialtyNinjutsu', 'uses', acctid, Ninjutsu FROM " . db_prefix("accounts");
			db_query($sql);
			debug("Dropping Ninjutsuuses field from accounts table");
			$sql = "ALTER TABLE " . db_prefix("accounts") . " DROP Ninjutsuuses";
			db_query($sql);
		}
	}
	debug("Migrating Ninjutsu Specialty");
...
function specialtyNinjutsu_uninstall(){
...
function specialtyNinjutsu_dohook($hookname,$args){
	...
	case "set-specialty":
		if($session['user']['specialty'] == $spec) {
			page_header($name);
			output("`5Growing up, you recall doing much better in the academy at Ninjutsu.");
			output("");
			output("");
		}
		break;
			...
			if (get_module_pref("uses") >= $l){
				...
				case 1:
					apply_buff('da1',array(
						...
						"schema"=>"module-specialtyNinjutsu"
					));
					break;
				case 2:
...
function specialtyNinjutsu_run(){
}
?>
