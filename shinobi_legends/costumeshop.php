...
function costumeshop_getmoduleinfo(){
	$info = array(
		...
		"settings"=>array(
			...
			"-- Costume 6 --,note",
			"costume6"=>"Sixth costume (max 25 chars)|`lU`xnderwear`l G`xnome",
			"weapon6"=>"Custom weapon for the 6th costume|Little Hands",
			"armor6"=>"Custom armor for the 6th costume|Bootcut Clothes",
...
function costumeshop_run(){
	...
	$cos_count = 6;
	if (get_module_setting("christmastxt")){
		output("`c`b`\$%s`\$'s Costume Shop`b`c`n",$owner);
	}else{
		output("`c`b`\$%s`\$'s Haunter's Lodge`b`c`n",$owner);
	}
	...
	if ($op == "return") {
		...
		costumeshop_changearmor("");
		debuglog("changed name to $oldname due to returning a costume");
		output("`n`7You decide to hand in the costume before it's due. Any days you had left are gone now.`n");
		page_footer();
	}	
	...
	if ($rleft > 0){
		...
		output("`7You could get rid of the costume prematurely, though. There is a wardrobe for deposit hidden in a dark corner...`n");
		addnav("Return");
		addnav("Return the costume","runmodule.php?module=costumeshop&op=return");
		...
		if (get_module_setting("christmastxt")){
			...
			output("`7The man laughs heartily and says, \"`)Happy Holidays, and welcome to my little shop! I am `\$%s`). Please, look around and see if you find something you like!`7\"`n`n",$owner);
		}else{
			...
			output("`7\"`)Greetings traveller! I am `\$%s`), and welcome to my shop!`7\"`n`n",$owner);
		}
