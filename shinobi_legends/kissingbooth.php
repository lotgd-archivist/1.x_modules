...
function kissingbooth_run()
{
	...
	if ($op == "") {
		while ($kissee == "") {
			$guys = array("`qSasuke", "`!Itachi", "`@Na`3ru`!to", "`6Rock Lee", "`%Ero-Sennin", "`&Neji", "`#Chouji", "`4Shikamaru","`)Gaara");
			$girls = array("`)Temari", "`7TenTen", "`QSa`\$ku`Qra", "`@Ta`&yu`@ya", "`5Hinata", "`!Ta`vko");
		...
		output("`QYou walk towards the booth marked \"`@Konoha Kissing Booth`Q\".`n");
		...
		if ($session['user']['prefs']['sexuality']==SEX_MALE) {
			$heshe = translate_inline("he");
			$Cheshe = translate_inline("He"); // "capital he/she." capital idea, wot?
			$hisher = translate_inline("his");
		}else{
			$heshe = translate_inline("she");
			$Cheshe = translate_inline("She");
			$hisher = translate_inline("her");
		}
		$rnd = e_rand(1,16);
		switch ($rnd){
		case 1: case 2: case 3: case 4: case 5:
			...
			output("%s`6 leans forward and kisses you gently on the lips.",
					$kissee);
			output("%s holds it slightly longer than you expected and pulls back with an impressed look on %s face.`n",$Cheshe,$hisher);
			output("\"`#Not bad`6\", %s says, \"come back anytime!\"`n",$heshe);
		...
		case 10: case 11: case 12:
			...
			output("%s`6's eyes light up as you approach, and %s kisses you with enthusiasm.`n", $kissee,$heshe);
			...
			output("`6You turn around to see %s`6 winking at you! \"`#Go get 'em, tiger!`6\", %s says.`n", $kissee,$heshe);
		...
		case 16:
			...
			output("`6When you come to, %s`6 is cradling your head in %s lap.`n", $kissee,$hisher);
			output("%s `6bends down and kisses you on the forehead, and sets you on your feet.`n",$Cheshe);
