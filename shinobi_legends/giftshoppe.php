...
function giftshoppe_dohook($hookname,$args) {
	...
	case "village":
		...
		if ($days <5 && $days > -4) { // christmas display in village
			rawoutput("<h2>");
			output("`\$'Tis the season to be jolly! `7Under the tree at the Inn, you see a pile of presents.");
			rawoutput("</h2>");
			if ($count == 0) {
	...
	case "village":
		...
		if ($days <= get_module_setting("daysprior") && $days >= 0) {
			if ($session['user']['location']!='Konohagakure') break;
