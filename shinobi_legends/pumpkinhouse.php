...
File:    pumpkin.php
Author:  Red Yates aka Deimos Edits by KainStrider to work in Haunted House
Date:    10/23/2004
...
*/
function pumpkinhouse_getmoduleinfo(){
	$info = array(
		"name"=>"Pumpkin Carving - Haunted Castle",
		"version"=>"1.0",
		"author"=>"`\$Red Yates, Edits by KainStrider for Haunted House",
		"download"=>"EORPG",
...
function pumpkinhouse_uninstall(){
	return true;
}

function pumpkinhouse_dohook($hookname, $args){
	global $session;
	switch ($hookname){
	case "newday":
...
function pumpkinhouse_run(){
	...
	if ($op==""){
		if ($tries<$perday){
			...
			if ($session['user']['gold']<$cost){
				...
				addnav(array("Carve a Pumpkin (`^%s`0 gold)",$cost),
						"runmodule.php?module=pumpkinhouse&op=carve");
			}
		}else{
		...
		addnav("Return to the Haunted Gardens","runmodule.php?module=hauntedhouse&op=hauntedhousegarden");
	}elseif ($op=="carve"){
		$tries++;
		...
		addnews("%s `Qcarved a pumpkin in the `5Haunted House Garden`Q.`0",$session['user']['name']);
		$result=e_rand(1,30);
