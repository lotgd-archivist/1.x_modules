...
/$resizedwidth));
					...
					set_module_pref('avatar_resize_dimensions',implode("|",array($resizedheight,$resizedwidth)),'avatar',$row['acctid']);
				}
				$image.=">";
				if (filter_var($url, FILTER_VALIDATE_URL)===false) {
					// url invalid
					if (filter_var("http://test.com".$picname,FILTER_VALIDATE_URL)===false) 
						$image = "Picture invalid! String given: ".htmlentities($url,ENT_COMPAT,'utf-8');
				}
			...
			default:
				...
				if (httpget('giveok') && $ok) {
					...
					require_once("./lib/systemmail.php");
					//0= Technical Deny, i.e. submitted a wrong link
					//1= Improper, i.e. sexual content
					$reasons=array(
						array("`^Your picture is not showing and it seems you did something wrong as you submitted it.`n`nPlease choose a new one and petition/mail the appropriate link to us.`n`n If you want to upload a new one and/or need the points back, please also do so,too.`n`nRegards`n%s`^`n `&Staff",$session['user']['name']),
						array("`^Your picture is not within the local policies/rules and was considered offensive.`n`n Please choose a new one and petition/mail the appropriate link to us.`n`nIf you want to upload a new one and/or need the points back, please also do so,too.`n`nRegards`n%s`^`n `&Staff",$session['user']['name']),

						);
					systemmail($user,array("`xYour avatar has `\$NOT`x been validated!"),$reasons[$reason]);
	...
	rawoutput("<tr class='trhead'><td>". translate_inline("Picture") ."</td><td>". translate_inline("Name") ."</td><td>".translate_inline("Ops")."</td></tr>");
				...
				$deny_technic=appoencode(translate_inline("`\$Deny due to technical stuff + Set Default Picture`0"));
				$deny_improper=appoencode(translate_inline("`\$Deny as being improper + Set Default Picture`0"));

				while ($row=db_fetch_assoc($result)) {
					$i=!$i;
					$url=get_module_pref("avatar","avatar",$row['acctid']);
					$image = "<a href='$url' target='_blank'><img src='$url' align='center' width='150' height='150'></a>";
					if (filter_var($url, FILTER_VALIDATE_URL)===false) {
						// url invalid
						if (filter_var("http://test.com".$url, FILTER_VALIDATE_URL)===false)
							$image = "Picture invalid! String given: ".htmlentities($url,ENT_COMPAT,'utf-8');
					}
					rawoutput("<tr class='".($i?"trlight":"trdark")."'><td>$image</td><td>");
					rawoutput("<a href='mail.php?op=write&to={$row['login']}' class='colLtGreen' target='_blank' onClick=\"".popup("mail.php?op=write&to={$row['login']}").";return false;\">".sanitize($row['name'])."</a></td><td>");
					...
					rawoutput("<div style='text-align:right;width:100%;'><a href='runmodule.php?module=avatar&op=validate&mode=&deny=1&reason=0&user={$row['acctid']}'>$deny_technic</a><br><br>");
					rawoutput("<a href='runmodule.php?module=avatar&op=validate&mode=&deny=1&reason=1&user={$row['acctid']}'>$deny_improper</a>");
				
					rawoutput("</div></td></tr>");
					addnav("","runmodule.php?module=avatar&op=validate&mode=&giveok=1&user={$row['acctid']}");
					addnav("","runmodule.php?module=avatar&op=validate&mode=&deny=1&reason=0&user={$row['acctid']}");
					addnav("","runmodule.php?module=avatar&op=validate&mode=&deny=1&reason=1&user={$row['acctid']}");
