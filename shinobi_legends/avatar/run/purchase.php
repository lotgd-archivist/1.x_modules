<?php
		...
		$av=get_module_pref('avatar');
		output("`xYou already have an avatar, located at this URL: ");
		if (substr($av,0,6)=="noava/") $av=str_replace("noava","avatars",$av);
		rawoutput("<a href='$av'>$av</a>");
		output_notl("`n`n");
		...
		if ($pointsavail >= $cost) {
			...
			if (get_module_setting("allowpersonal")) {
				addnav("Avatar by Weblink","runmodule.php?module=avatar&op=personal&cost=$cost");
				output("`\$Avatar by Weblink`g: You can also enter an URL to point to an avatar of your choice somewhere on the web.`7`n`n");
			}
			if (get_module_setting("allowupload")) {
				addnav("Avatar by Upload","runmodule.php?module=avatar&op=upload&cost=$cost");
				output("`\$Avatar by Upload`l: You can also upload files up to a size of `v%s`l bytes for use on this server, the pictures cannot be linked from the outside.`n`n`7",get_module_setting('uploadsize'));
			}
