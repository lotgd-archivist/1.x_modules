		...
		switch($mode) {
			case "invalidate":
				...
				if ($who=='') {
					...
					set_module_pref('avatar_resize_dimensions','','avatar',$who); //new sizes
			...
			case "validate":
				...
				if (httpget('giveok')) {
					...
					set_module_pref("validated",1,"avatar",$who);
					require_once("./lib/systemmail.php");
					systemmail($who,array("Your avatar has been validated!"),array("`^As of now, your avatar will be visible.`n`nRegards`n%s`^`n `&Staff",$session['user']['name']));
					set_module_pref('avatar_resize_dimensions','','avatar',$who); //new sizes
				} else {
					output_notl("`n`n");
				}
...
/$resizedwidth));
			...
			default:
				...
				if (httpget('giveok') && $ok) {
					...
					debuglog(sprintf("Avatar was validated by %s - mailed",sanitize($session['user']['name'])),sanitize($user),sanitize($user));
